...
def get_pair_batch_train_common(args, dataset, n_batch, train_idx, batch_size, num_neighbor):
    all_triples = dataset.train_data #一半正样本，一半负样本
    # all_triples_labels = torch.cat((all_triples, labels), dim=1)
    # print('all_triples_labels.shape', all_triples_labels.shape)
    if n_batch == 0: #第一个batch
        np.random.shuffle(train_idx)
    length = batch_size
    if (n_batch + 1) * batch_size > len(train_idx):# 最后一个batch
        ids = train_idx[n_batch * batch_size:]
        length = len(train_idx) - n_batch * batch_size
    else:#除了第一次和最后一次，中间的那几次batch
        ids = train_idx[n_batch * batch_size: (n_batch + 1) * batch_size]

    assert length == len(ids), "ERROR: batch_size != length."
    # ids 是一个 batch的所有id的list
    n = all_triples.size(0) // 2 # 所有正样本的数量
    sample_triple = []
    for i in range(len(ids)):
        idx = ids[i]
        temp = [all_triples[idx], all_triples[idx + n]] # 一个正样本拼接一个负样本
        sample_triple += temp #这里的sample_triple 是一个正样本接上一个负样本，所以长度为原来的2倍
    # idx = random.randint(0, n - 1)
    # sample_triple_labels = [all_triples_labels[idx], all_triples_labels[idx + n]]
    batch_triples = []
    for i in range(len(sample_triple)): #这里的正样本和负样本都获得3元组的邻居
        #get_triple_neighbor函数就是获得共同头实体和共同尾实体的所谓的邻居
        #这里默认是一个原使三元组+39个共同头实体的邻居，再加上一个原始三元组+39个共同尾实体的邻居，一共80个,同时，一个正样本对应一个负样本，所以80还要*2，一共256*160个数据
        hrt_neighbor = get_triple_neighbor(sample_triple[i][0].item(), sample_triple[i][1].item(), sample_triple[i][2].item(), dataset, num_neighbor)
        batch_triples = batch_triples + hrt_neighbor
    ...
    label=[]
    for id in ids:
        temp_triple=all_triples[id]
        head_embedding=dataset.id2feature[int(temp_triple[0])]
        relation_embedding=dataset.rel2feature[int(temp_triple[1])]
        tail_embedding=dataset.id2feature[int(temp_triple[2])]
        bert_embedding.append([head_embedding,relation_embedding,tail_embedding])
    for id in ids:
        #print(dataset.triples[id][3])
        label.append(dataset.triples[id][3])
    return batch_h, batch_r, batch_t, length,bert_embedding,label

def get_pair_all_test_common(args, dataset,  train_idx, batch_size, num_neighbor):# 这里需要注意一下，batchsize是最后测试数据的长度
    all_triples = dataset.train_data #一半正样本，一半负样本
    # all_triples_labels = torch.cat((all_triples, labels), dim=1)
    # print('all_triples_labels.shape', all_triples_labels.shape)
    # if n_batch == 0: #第一个batch
    ...
    return batch_h, batch_r, batch_t,bert_embedding,label,batch_size
