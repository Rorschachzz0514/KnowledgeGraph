...
class Reader:
    ...
    def get_add_wiki_ent_id(self, ent,label,description):
    ...
    def get_add_wiki_rel_id(self, rel,label,description):
        if rel in self.rel2id:
            rel_id = self.rel2id[rel]
        else:
            rel_id = len(self.rel2id)
    ...
    def read_triples_wiki(self):
        ...
        with open(self.path, "rb") as file:
            ...
            for list in loaded_list:
                ...
                label=list[9]
                head_id = self.get_add_wiki_ent_id(head,head_label,head_description)
                rel_id = self.get_add_wiki_rel_id(rel,relation_label,relation_description)
                tail_id = self.get_add_wiki_ent_id(tail,tail_label,tail_description)
